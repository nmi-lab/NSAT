...
def load_mnist(data_url, labels_url, n_samples=None, nc_perlabel=1,
               with_labels=True, randomize=False, binary=False, seed=None,
               labels_offset=0, **kwargs):
    '''
    Loads MNIST data. Returns randomized samples as pairs [data vectors, data
    labels]
    test: use test data set. If true, the first n_sample samples are used
    (no randomness)
    ...
    iv = np.fromstring(f_image.read(Nimages * nbyte_per_image),
                       dtype='uint8').reshape(Nimages, nbyte_per_image).astype('float') / 256
    ...
    if randomize is False:
        ...
        iv_seq, iv_l_seq = select_equal_n_labels(
            n_samples, iv_clamped, iv_l, seed=seed)
    ...
    data_vectors = np.concatenate(
        [iv_seq, np.zeros([iv_seq.shape[0], labels_offset]),
         iv_label_seq], axis=1)
...
def create_spike_train(data_train, t_sample, scaling=1, n_mult=1,
                       with_labels=True):
    data_train = np.array(data_train)
    data = np.concatenate([data_train for _ in range(n_mult)])
    t_sim = len(data) * t_sample * n_mult
    return SimSpikingStimulus(scaling * data, t_sample, t_sim=t_sim,
